...
class _ShellScreenState extends State<ShellScreen> {
  ...
  Widget build(BuildContext context) {
    if (_showNotifications) {
      return WillPopScope(
        onWillPop: () async { setState(() => _showNotifications = false); return false; },
    ...
    return Scaffold(
      body: IndexedStack(
        index: _currentIndex,
        children: [
          HomeScreen(onNotifications: () => setState(() => _showNotifications = true)),
          ...
          child: Row(
            ...
            children: [
              ...
              _NavItem(icon: Icons.calendar_today_outlined, activeIcon: Icons.calendar_today_rounded,
                  label: 'Schedule', index: 1, current: currentIndex, onTap: onTap),
              _NavItem(icon: Icons.history_outlined,  activeIcon: Icons.history_rounded,
                  label: 'History',  index: 2, current: currentIndex, onTap: onTap),
              _NavItem(icon: Icons.person_outline,    activeIcon: Icons.person_rounded,
                  label: 'Profile',  index: 3, current: currentIndex, onTap: onTap),
            ],
          ),
        ),
      ),
      ...
      child: AnimatedContainer(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(active ? activeIcon : icon,
              size: 22, color: active ? AppColors.primary : AppColors.outline),
          if (active) ...[
